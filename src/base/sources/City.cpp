...
City::City(const int id, const double x, const double y)
    : id(id)
    , x(x)
    , y(y)
{
...
{ // Overload para comparação de city==city
...
{ // Overload para impressão da população
    output << c.id;
    return (output);
}

const City& City::operator=(const City& c)
{ // Overload de atribuição city=city
    this->x = c.getX();
    this->y = c.getY();
    this->id = c.getId();
    return (c);
