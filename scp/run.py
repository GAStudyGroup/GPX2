            ...
            for pop_size in c.POP_SIZE:
                for tour_name in c.TOURS:
                    for i in range(c.NUMBER_OF_RUNS):
                        ...
                        call(["./"+c.BIN_PATH+"GPX2", "-n "+tour_name, "-l "+c.LIBS_PATH, "-s "+str(pop_size), "-id "+str(i), "-lk "+str(kl_percentage), "-np "+str(new_pop_type), "-nb "+str(n_best), "-bf "+str(best_fitness),"-r "+str(c.RESET_PERCENTAGE)])
